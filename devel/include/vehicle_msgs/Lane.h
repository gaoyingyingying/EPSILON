  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::vehicle_msgs::Lane_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.child_id.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.child_id[i]);
    }
    if (v.child_id.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.father_id.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int32_t>::stream(s, true ? std::string() : indent + "    ", v.father_id[i]);
    }
    if (v.father_id.empty() || true)
      s << "]";
    if (true || !indent.empty())
