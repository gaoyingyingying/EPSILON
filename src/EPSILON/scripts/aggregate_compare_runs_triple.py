...
def calc_stats(values):
    ...
    return {"count": n, "mean": mean, "variance": variance, "std": std}
...
def main():
    parser = argparse.ArgumentParser(description="Aggregate repeated EUDM-AI/EUDM-BASE/MPDM runs")
    ...
    aggregate = {"runs": len(runs), "methods": list(METHODS), "metrics": {}}
    ...
    with csv_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["metric", "method", "count", "mean", "variance", "std", "mean_pm_std"])
        for metric in METRICS:
            for method in METHODS:
                stats = aggregate["metrics"][metric][method]
                writer.writerow(
                    [metric, method, stats["count"], stats["mean"], stats["variance"], stats["std"], format_mean_std(stats)]
                )
    ...
    display_name = {"eudm_ai": "EUDM_AI", "eudm_base": "EUDM_BASE", "mpdm": "MPDM"}
